        ...
        return (new TreeBuilder('settings'))->getRootNode()
            ...
            ->children()
                ...
                ->arrayNode(SettingsConfiguration::NOTIFICATIONS)
                    ...
                    ->children()
                        ...
                        ->arrayNode(NotificationsConfiguration::GPS_CONFIG)
                            ->children()
                                ->scalarNode('transport')->defaultValue('rest')->end()
                ...
                ->arrayNode(SettingsConfiguration::ELASTIC_LANGUAGE_DICTIONARIES_KEY)
                    ...
                    ->end()
                ->end()
                ->scalarNode(SettingsConfiguration::URL_FILE_TRUSTED_DOMAINS_KEY)
                    ->defaultValue('')
                    ->info('Comma-separated domain patterns trusted for URL file download (e.g. ".sme.sk,sme.local")')
                ->end()
                ->booleanNode(SettingsConfiguration::URL_FILE_ALLOW_PRIVATE_NETWORKS_KEY)
                    ->defaultFalse()
                    ->info('Dev-only escape hatch: lets trusted domains resolve to private IPs (e.g. sme.local). Keep false in production.')
                ->end()
