...
* Add sys endpoint `POST /api/sys/v1/image/usage` (`scopeResourceName`, `scopeResourceId`, optional `holderResourceName`/`holderResourceId`, `damIds` max 1000) — declarative usage sync: the caller states which photos one scope holds, DAM claims them for the whole take-over group, reports photos held by another scope as `conflicts` (with the current holder, HTTP 200) and releases everything the scope held but the request omits; DAM is now the source of truth for single use exclusivity. Adds `AssetFileAttributes::$usedByResourceName`/`$usedByResourceId` (serialized in the adm asset file attributes, list and detail) and the internal `$usedByScopeName`/`$usedByScopeId` — hosts need a migration adding four `VARCHAR(64) DEFAULT '' NOT NULL` columns to `asset_file` plus the `IDX_attributes_used_by_scope` index. `firstUsedAt` is untouched by the sync and is now also exposed in the adm asset file attributes
