...
* Add sys endpoint `POST /api/sys/v1/image/take-over` (`imageFile`, optional `targetAssetLicence`, `force`): answers which image file the caller may use — the requested one when its licence allows direct use, otherwise a copy taken over into the target licence, finished synchronously so the returned file is usable immediately. Without a target licence it only reports the verdict (`image_direct_use_disabled` for a licence that forbids direct use), a file already present in the target licence is reused unless the source is single use (`image_take_over_conflict`), and a source that is not its asset's processed main file is refused (`image_take_over_source_invalid`)
* `AssetFileFirstUseFacade` also stamps the take-over origin, so the licence clock starts with the first use of any copy of the photo; an origin already deleted by retention is skipped
* First use is recorded by DAM itself: `ImageUseFacade::useImages()` stamps `firstUsedAt` (write-once, on the file and its take-over root) for every file of the batch, so the licence clock starts with the first successful `use` instead of waiting for the host to report a publish. `AssetFileFirstUseFacade::record()` is the only entry point — there is no sys endpoint for reporting a first use, because the use call already knows it
